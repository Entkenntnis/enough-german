  ...
  // Simple touch support (primary touch only). Keeps code small vs Pointer Events.
  ...
  this.domElement.addEventListener( 'touchcancel', this._onTouchEnd , false );
  ...
  */
  ...
  ObjectControls.prototype._getTouch = function( e ){
    if( e.touches && e.touches[0] ) return e.touches[0];
    if( e.changedTouches && e.changedTouches[0] ) return e.changedTouches[0];
    return null;
  }

  ObjectControls.prototype.onTouchStart = function( e ){
    if( this.leap ) return; // don't mix with Leap
    if( this._touchActive ) return; // ignore second finger
    var t = this._getTouch( e );
    if( !t ) return;
    this._touchActive = true;
    this.mouseMove( t ); // prime position
    this.mouseDown();
  }

  ObjectControls.prototype.onTouchMove = function( e ){
    if( this.leap ) return;
    if( !this._touchActive ) return;
    var t = this._getTouch( e );
    if( !t ) return;
    // Only prevent default (e.g., scrolling) when in fullscreen to allow normal page scroll otherwise
    if( document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement ){
      e.preventDefault(); // prevent page scroll during interaction in fullscreen
    }
    this.mouseMove( t );
  }

  ObjectControls.prototype.onTouchEnd = function( e ){
    if( this.leap ) return;
    if( !this._touchActive ) return;
    this.mouseUp();
    this._touchActive = false;
