  ...
  function ObjectControls( eye , hand, controller , params ){
    ...
    // Legacy mouse listeners (kept as fallback for very old browsers / non-pointer devices)
    ...
    /*
      Phase 1 Pointer Events support:
      We add pointer listeners alongside existing mouse listeners to support touch, pen, and unified mouse handling.
      Future phase can remove the legacy mouse* once confidence is high.
    */
    this.activePointerId = null; // Tracks the primary pointer we respond to

    // Bind once so we can potentially remove later if needed
  ...
  */
  ...
  /*
    POINTER EVENT HANDLERS (Phase 1)
    These wrap existing mouse* methods so core logic remains untouched.
    We ignore pointer events while Leap is active (this.leap) to avoid conflicts.
  */

  ObjectControls.prototype.onPointerDown = function( event ){
    if( this.leap ) return; // Leap interaction takes precedence
    // Only capture first active pointer (for touch we ignore additional fingers in this phase)
    if( this.activePointerId === null ){
      this.activePointerId = event.pointerId;
      // Update position immediately (some browsers won't emit move if finger doesn't move before up)
      this.mouseMove( event );
      this.mouseDown();
    }
  }

  ObjectControls.prototype.onPointerMove = function( event ){
    if( this.leap ) return;
    if( this.activePointerId === null ){
      // Allow hover for mouse / pen without button down; only for fine pointers
  ...
  ObjectControls.prototype.onPointerUp = function( event ){
    if( this.leap ) return;
    if( event.pointerId !== this.activePointerId ) return;
    this.mouseUp();
  ...
  ObjectControls.prototype.onPointerCancel = function( event ){
    if( this.leap ) return;
    if( event.pointerId !== this.activePointerId ) return;
    // Treat like an up to avoid stuck selection
    this.mouseUp();
    this.activePointerId = null;
  }

  ObjectControls.prototype.onPointerLeave = function( event ){
    if( this.leap ) return;
    if( event.pointerId !== this.activePointerId ) return;
    // If pointer leaves element while pressed, finalize interaction
    this.mouseUp();
    this.activePointerId = null;
  }
