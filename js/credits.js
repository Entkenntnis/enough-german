...
credits.activateTail = function (n) {
  ...
  G.v2.set(Math.random() - 0.5 * 300, Math.random() - 0.5 * 300, 300)
...
credits.addToInitArray(
  function () {
    ...
    this.links.push(new Donate('DONATE', 'https://www.wearebgc.org/', 230))
    this.links.push(
      new Donate(
        'TWITTER',
        "http://twitter.com/share?text='ENOUGH'%20by%20%40cabbibo%20:%20An%20interactive%20audio/visual%20odyssey%20in%20the%20guise%20of%20a%20picture%20book%20&url=http://cabbi.bo/enough",
        80
      )
    )
    this.links.push(
      new Donate(
        'FACEBOOK',
        'http://www.facebook.com/sharer.php?u=http://cabbi.bo/enough',
        80
      )
    )
    ...
    for (var i = 0; i < this.links.length; i++) {
      ...
      if (i == 0) {
        ...
        var pos = new THREE.Vector3((i - 0.6) * 200, -200, -500)
        pos.add(G.v1)
        this.links[i].start(this.scene, pos)
      }
    }
      ...
      start: function () {
        for (var i = 0; i < this.page.audio.length; i++) {
          if (
            ...
            this.page.audio[i].gain.gain.value = 0.4
          }
        }
      },
    })
    ...
    this.sectionParams.push({
      ...
      start: function () {
        for (var i = 0; i < this.audio.length; i++) {
          this.audio[i].gain.gain.value = 0.4
        }
        ...
        for (var i = 0; i < this.furryTails.length; i++) {
          ...
          fT.velocity.x = (Math.random() - 0.5) * 200
          fT.velocity.y = (Math.random() - 0.5) * 200
          fT.velocity.z = (Math.random() - 0.5) * 200

          fT.removeAllForces()

          if (i !== 0) {
            fT.addDistanceForce(this.furryTails[i - 1].position, 0.0004)
          } else {
            fT.addDistanceForce(G.sol.position.relative, 0.004)
          }
        }
      }.bind(credits),
    })
...
credits.addToStartArray(
  ...
  */
    ...
    for (var i = 0; i < this.choreography.length; i++) {
      ...
      color.setHSL(Math.pow(i / this.choreography.length, 3), 1, 0.3)
